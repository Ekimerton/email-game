...
describe('AMP HTML Validation', () => {
  ...
  it('should validate rendered GET / endpoint HTML with theme=dark as valid AMP4EMAIL', async () => {
    const { app } = await import('../src/index')
    const res = await app.request('/?email=test@company.com&forceHttps=true&theme=dark')
    const renderedHtml = await res.text()
    ...
    if (result.status !== 'PASS') {
      const errorDetails = result.errors
        .map(e => `[Line ${e.line}:${e.col}] ${e.message} ${e.specUrl || ''}`)
        .join('\n')
      expect.fail(`AMP Validation Failed for rendered dark mode GET / HTML:\n${errorDetails}`)
    }

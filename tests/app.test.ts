...
describe('Email Signup Landing Page & Subscribe API', () => {
  ...
  it('should show subscribed success confirmation when subscribed=true on GET /', async () => {
    ...
    expect(html).toContain("You'll get emails at 9:00 AM every day.")
    expect(html).not.toContain("Play Today's Puzzle Online")
  })

  it('should include the AMP/Apple Mail disclaimer, 9:00 AM delivery, and omit emojis from benefits', async () => {
    const res = await app.request('/')
    expect(res.status).toBe(200)
    const html = await res.text()
    ...
    expect(html).toContain('Every morning at 9:00 AM, right inside your email.')
    expect(html).not.toContain('feature-icon')
  ...
  it('should include dedicated video card spot for demo-4 without loop and 16px input styling to avoid iOS zoom', async () => {
    const res = await app.request('/')
    expect(res.status).toBe(200)
    const html = await res.text()
    expect(html).toContain('video-card')
    expect(html).toContain('id="demo-video"')
    expect(html).toContain('src="/demo-4.mp4"')
    expect(html).toContain('src="/demo.mp4"')
    expect(html).not.toContain('src="/demo-2.mp4"')
    expect(html).not.toContain('src="/demo-3.mp4"')
    expect(html).not.toContain('src="/demo-recording')
    ...
    const demo4Res = await app.request('/demo-4.mp4')
    expect(demo4Res.status).toBe(200)
    expect(demo4Res.headers.get('content-type')).toBe('video/mp4')

    const demoRes = await app.request('/demo.mp4')
    expect(demoRes.status).toBe(200)
    expect(demoRes.headers.get('content-type')).toBe('video/mp4')
  })

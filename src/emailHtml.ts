...
<body>
    <div class="email-container">
        ...
        <div class="game-body">
            ...
            <!-- Definitions Section - Native HTML for automatic responsive wrapping -->
            <div class="definitions-section">
                <div class="section-label">
                    <span>Revealed Definitions</span>
                    <span [text]="(revealedCount || gameState.revealedCount) + ' of 5'">1 of 5</span>
                </div>
                <div class="definitions-cards">
                    __DEFINITIONS_CARDS__
                </div>
            </div>

            <!-- Dynamic State Section - Letter Hints, Guesses & Message Banner -->
            <amp-list id="stateList" width="auto" height="104" layout="fixed-height"
            ...
            <div class="form-container">
                <form id="guess-form" method="POST" action-xhr="https://email-game.teamify.workers.dev/api/guess?email=USER_EMAIL_PLACEHOLDER&date=USER_DATE_PLACEHOLDER"
                    on="submit-success:AMP.setState({ gameState: event.response, revealedCount: event.response.revealedCount }),guess-form.clear,stateList.refresh,leaderboardList.refresh;
                        submit-error:AMP.setState({ gameState: event.response, revealedCount: event.response.revealedCount }),stateList.refresh">
                ...
                <form id="hint-form" method="POST" action-xhr="https://email-game.teamify.workers.dev/api/hint?email=USER_EMAIL_PLACEHOLDER&date=USER_DATE_PLACEHOLDER" hidden
                    on="submit-success:AMP.setState({ gameState: event.response, revealedCount: event.response.revealedCount }),stateList.refresh;
                        submit-error:AMP.setState({ gameState: event.response, revealedCount: event.response.revealedCount }),stateList.refresh">
                    <input type="hidden" name="email" value="USER_EMAIL_PLACEHOLDER">
                </form>
            </div>
